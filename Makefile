...
submission-files = COPYING Makefile \
  notes.txt  *.c *.h randall  # More files should be listed here, as needed.
...
check5: 
	if [ `./randall -i mrand48_r -o 100 500 | wc -c` -eq 500 ]; \
	then echo "test passed -> ./randall 500 "; \
	else echo "test failed -> ./randall 500 "; \
	fi;

check6: 
	if [ `./randall -i mrand48_r -o stdio 500 | wc -c` -eq 500 ]; \
	then echo "test passed -> ./randall 500 "; \
	else echo "test failed -> ./randall 500 "; \
	fi;
